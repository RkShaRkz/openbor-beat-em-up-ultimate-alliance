...
{//Adjust velocity in Rage Run Hit
	...
	if(vAniID == openborconstant("ANI_FREESPECIAL5")){
		changeentityproperty(self, "velocity", xVel/1.3, 0, 0);
		changeentityproperty(target, "velocity", xVel/2.6, 0, 0);
	}
	
	if(vAniID == openborconstant("ANI_FOLLOW4")){
		if(THeight <= 0){
			changeentityproperty(self, "velocity", xVel/1.6, 0, 0);
			changeentityproperty(target, "velocity", xVel/1.6, 0, 0);
		}
	}
}
